## Interesting issues


#### DNS resolv.conf issue in alpine
https://github.com/gliderlabs/docker-alpine/issues/8

#### Running kubernetes in Docker containers
https://github.com/kubernetes/kubernetes/issues/13424


#### Experiment run kubelet as docker container
https://github.com/kubernetes/kubernetes/issues/4869

#### Allow controlling the mount namespace
https://github.com/docker/docker/issues/10088


#### Getting started for docker single-node cluster should be easier
https://github.com/kubernetes/kubernetes/issues/12722

#### RancherOS on Raspberry Pi 2
https://github.com/rancher/os/issues/417

#### Compile Kubernetes with go1.5.1
https://github.com/kubernetes/kubernetes/issues/16229
...
DOCKER_BRANCH=${DOCKER_BRANCH:-"v1.8.3"}

# Download docker
git clone -b $DOCKER_BRANCH --single-branch https://github.com/docker/docker.git ~/docker
cd ~/docker
...
#sed -e "s@armv7/armhf-ubuntu:14.04@resin/rpi-raspbian:jessie@" -i Dockerfile
